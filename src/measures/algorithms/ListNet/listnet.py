...
import src.measures.algorithms.ListNet.net as net
...
class ListNet(object):
    ...
    def test(self, ranking, filename,noscore = True):
        testres = 0
        ...
        if noscore:
            ...
            for j in range(self.test_T):
                ...
                pred_j = self.predict(x_j)
                pred_j = F.reshape(pred_j,(pred_j.data.shape[0],))
                for elem, candidate in zip(pred_j, ranking[indexMin : indexMax]):
