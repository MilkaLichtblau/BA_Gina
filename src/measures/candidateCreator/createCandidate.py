...
import pandas as pd
import numpy as np
from measures.candidateCreator.candidate import Candidate
...
class createCandidate():
    ...
    def createLearningCandidate(filename):
        ...
        queryNumber = data[globals.query]

        queryNumber = queryNumber.unique()

        for query in queryNumber:
            dataQuery = data.loc[data.query_id == query]
            ...
            for row in dataQuery.itertuples():
                ...
                if float(row[data.columns.get_loc(globals.protected_attribute) + 1]) == 0.0:
                    nonProtected.append(Candidate(1 - float(row[data.columns.get_loc(globals.rank) + 1])/l, 1- float(row[data.columns.get_loc(globals.rank) + 1])/l, [], float(row[data.columns.get_loc(globals.rank) + 1]), row[data.columns.get_loc(globals.query) + 1], features))
                else:
                    protected.append(Candidate(1 - float(row[data.columns.get_loc(globals.rank) + 1])/l, 1 - float(row[data.columns.get_loc(globals.rank) + 1])/l, "protectedGroup", float(row[data.columns.get_loc(globals.rank) + 1]), row[data.columns.get_loc(globals.query) + 1], features))
